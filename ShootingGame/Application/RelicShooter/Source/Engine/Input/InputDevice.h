...
struct DigitalInput
{
	...
	void SetPressed(const bool pressed) { if (pressed) { m_State = m_State == E_ButtonState_Up ? E_ButtonState_Pressed : E_ButtonState_Down;  } else { m_State = E_ButtonState_Up; } }
...
public:
	...
	const DigitalInput& GetButtonInput(const E_DigitalInput button) const { return m_DigitalInputs[button]; }
	const AnalogueInput& GetAnalogueInput(const E_AnalogueInput trigger) const { return m_AnalogueInputs[trigger]; }
